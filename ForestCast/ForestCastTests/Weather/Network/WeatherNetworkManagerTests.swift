...
class WeatherNetworkManagerTests: XCTestCase {
    ...
    func testDecodeForecastWeatherData_at6pm() {
        ...
        let expectedResult = ForecastWeatherModel(forecastDays: get6pmTimes())
    ...
    func testDecodeForecastWeatherData_at4pm_showClosestWeatherInfo() {
        ...
        let expectedResult = ForecastWeatherModel(forecastDays: get3pmTimes())
    ...
    func testDecodeForecastWeatherData_noDataForCurrentDay() {
        ...
        let expectedResult = ForecastWeatherModel(forecastDays: get3pmTimesNoData())
        let result = sutNoData.decodeForecastWeatherData(data, currentHour: 16)
        XCTAssertEqual(result, expectedResult)
    }
    
    func testGetFilteredForecastDaysBasedOnClosestTime_laterTime() {
        let result = sut.getFilteredForecastDaysBasedOnClosestTime(allForecastDays: get3pmAnd6pmTimes(), closestHour: 18)
        XCTAssertEqual(result, get6pmTimes())
    }
    
    func testGetFilteredForecastDaysBasedOnClosestTime_earlierTime() {
        let result = sut.getFilteredForecastDaysBasedOnClosestTime(allForecastDays: get3pmAnd6pmTimes(), closestHour: 16)
        XCTAssertEqual(result, get3pmTimes())
    }
    
}

extension WeatherNetworkManagerTests {
    
    func get3pmAnd6pmTimes() -> [ForecastDays] {
        return get3pmTimes()+get6pmTimes()
    }
    
    func get6pmTimes() -> [ForecastDays] {
        ...
        return [forecastDayOne, forecastDayTwo, forecastDayThree, forecastDayFour, forecastDayFive]
    }
    
    func get3pmTimes() -> [ForecastDays] {
    ...
    func get3pmTimesNoData() -> [ForecastDays] {
